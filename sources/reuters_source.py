"""
Reuters Business RSS via Wayback Machine — free, no API key required.
Reuters RSS was archived multiple times per day on archive.org.
"""
...
RSS_FEEDS = [
    ...
    "feeds.reuters.com/reuters/companyNews",
]
...
class ReutersSource(BaseSource):
    ...
    def _parse_rss(self, snapshot_url: str, date: str) -> List[Article]:
        ...
        try:
            ...
            print(f"[Reuters] Failed to parse RSS {snapshot_url}: {e}")
    ...
    def fetch(self, date: str) -> List[Article]:
        ...
        for feed_url in RSS_FEEDS:
            print(f"[Reuters] Looking up RSS: {feed_url} for {date}...")
            snapshot_url = self._get_rss_snapshot(feed_url, date)

            if snapshot_url:
                print(f"[Reuters] Found snapshot: {snapshot_url}")
                ...
                print(f"[Reuters] No snapshot for {feed_url} on {date}")

            time.sleep(0.5)

        print(f"[Reuters] {len(all_articles)} unique articles found for {date}")
        return all_articles
