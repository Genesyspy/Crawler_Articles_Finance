"""
MarketWatch RSS via Wayback Machine — free, no API key required.
Fetches archived MarketWatch RSS feeds from archive.org for a given date.
RSS gives structured data (title, URL, description) vs raw HTML scraping.
"""
...
RSS_FEEDS = [
    ...
    "feeds.marketwatch.com/marketwatch/marketpulse/",
    "feeds.marketwatch.com/marketwatch/realtimeheadlines/",
]
...
class MarketWatchSource(BaseSource):
    ...
    def _parse_rss(self, snapshot_url: str, date: str) -> List[Article]:
        ...
        try:
            ...
            print(f"[MarketWatch] Failed to parse RSS {snapshot_url}: {e}")
    ...
    def fetch(self, date: str) -> List[Article]:
        ...
        for feed_url in RSS_FEEDS:
            print(f"[MarketWatch] Looking up RSS: {feed_url} for {date}...")
            snapshot_url = self._get_rss_snapshot(feed_url, date)

            if snapshot_url:
                print(f"[MarketWatch] Found snapshot: {snapshot_url}")
                ...
                print(f"[MarketWatch] No snapshot for {feed_url} on {date}")

            time.sleep(0.5)

        print(f"[MarketWatch] {len(all_articles)} unique articles found for {date}")
        return all_articles
